...
async def handle_generic_tcp(client_reader, client_writer):
    ...
    allowed, reason = await mitigator.check_and_mitigate(client_ip)
    if not allowed:
        logger.warning(f"[TCP-GENERIC] Bağlantı reddedildi: {client_ip} ({reason})")
    ...
    try:
        if not await mitigator.increment_connection(client_ip):
             logger.warning(f"[TCP-GENERIC] Bağlantı reddedildi (limit aşıldı): {client_ip}")
             client_writer.close(); await client_writer.wait_closed()
             return

        # <--- DÜZELTME: Hedef IP, "127.0.0.1" yerine dinamik olarak bulunan orijinal IP olarak ayarlandı.
        ...
        await mitigator.decrement_connection(client_ip)
