...
class HTTPDDoSMitigator:
    ...
    async def proxy_handler(self, req):
        ...
        allowed, reason = await self.mitigator.check_and_mitigate(ip)
        if not allowed:
        ...
        # <--- DÜZELTME: Hedef URL, "127.0.0.1" yerine dinamik olarak bulunan orijinal IP ile oluşturuldu.
        upstream_url = f"{req.scheme}://{original_dest_ip}:{original_dest_port}"

        if not await self.mitigator.increment_connection(ip):
        ...
        try:
            ...
            await self.mitigator.decrement_connection(ip)
