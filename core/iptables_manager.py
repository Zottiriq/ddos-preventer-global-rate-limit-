...
def setup_transparent_proxy_rules():
    ...
    for port, proto_type in config.TARGET_PORTS.items():
        redirect_port = config.HTTP_PROXY_LISTEN_PORT if proto_type == 'http' else config.GENERIC_TCP_LISTEN_PORT

        # <--- DÜZELTME: '-m owner' modülü PREROUTING zincirinde çalışmaz. Gelen paketlerin
        # henüz bir yerel sahibi (UID) yoktur. Bu kural kaldırıldı.
        _run_shell([
